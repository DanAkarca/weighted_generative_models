function [b c] = weight_optimised_generative_model(A,D,m,modeltype,modelvar,params,epsilon,weightupdate)
...
switch modeltype
    ...
    case 'matching'
        ...
        for iparam = 1:nparams
            ...
            eta = params(iparam,1);
            gam = params(iparam,2);
            [b(:,iparam) c(:,:,:,iparam)] = fcn_matching(A,Kseed,D,m,eta,gam,modelvar,epsilon,weightupdate);
            time = toc;
        end
...
function [b c] = fcn_matching(A,K,D,m,eta,gam,modelvar,epsilon,weightupdate)
K = K + epsilon;
n = length(D);
mseed = nnz(A)/2;
mv1 = modelvar{1};
...
for ii = (mseed + 1):m
    ...
    if weightupdate == 1;
        t(ii) = tic;
        % other conditions to speed it up
        if floor(ii)==(ii); % e.g. every 4 connections
           % take the subgraphs and minimise communicability for each
           % get components of the networks  
           [w fval exit output lamda grad hessian] = fmincon(@(A)sum((diag(sum(A,2))^-.5)*A*(diag(sum(A,2))^-.5),'all'),rand(n));
           A = abs(A.*w);
           %%%%% ***** CONSTRAIN THE WEIGHTS TO WHAT EXIST %%%%% *****
           %%%%% ***** ADD WEAK WEIGHTS %%%%% ******
           %%%%% ***** don't optimise completely, only a "bit" because otherwise too jumpy %%%%% *****
           %%%%% e.g., add a bias that prevents the communicability
           %%%%% being...?
           %%%%% minimised too much!
        end
        toc
    end
    % keep the update matrices over time
    c(:,:,ii) = A;
end
b = find(triu(A,1));
